(function EagerForgeChest() {
    ...
    // Register a block that opens the GUI on right-click
    ModAPI.addEventListener("bootstrap", function() {
        var BlockClass = ModAPI.reflect.getClassById("net.minecraft.block.Block");
        var ChestBlock = ModAPI.reflect.getSuper(BlockClass, x => x.length === 1);
        function CustomChestBlock() { ChestBlock(this); }
        ModAPI.reflect.prototypeStack(BlockClass, CustomChestBlock);
        
        CustomChestBlock.prototype.$onBlockActivated = function(world, pos, state, player) {
            if (!world.$isRemote) {
                player.$displayGUIChest(ModAPI.items["custom_chest"]);
            }
            return true;
        };
        
        var blockInstance = new CustomChestBlock();
        blockInstance.$setUnlocalizedName(ModAPI.util.str("custom_chest_block"));
        BlockClass.staticMethods.registerBlock.method(ModAPI.keygen.block("custom_chest_block"), ModAPI.util.str("custom_chest_block"), blockInstance);
    });
})();
