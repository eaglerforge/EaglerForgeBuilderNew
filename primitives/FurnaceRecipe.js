    ...
    function $$registerFurnaceRecipe() {
        function $$internalRegister() {
            try {
                ...
                const ItemStackCtorFromBlock = ItemStack.constructors.find(c => c.parameterTypes.length === 2 && 
                    c.parameterTypes[0].name === "net.minecraft.block.Block");
                const ItemStackCtorFromItem = ItemStack.constructors.find(c => c.parameterTypes.length === 2 && 
                    c.parameterTypes[0].name === "net.minecraft.item.Item");
                
                const FurnaceRecipes = ModAPI.reflect.getClassByName("FurnaceRecipes") || 
                                     ModAPI.reflect.getClassById("net.minecraft.item.crafting.FurnaceRecipes");
                const FurnaceRecipesInstance = ModAPI.util.wrap(FurnaceRecipes.staticVariables.smeltingBase || 
                                                              FurnaceRecipes.staticMethods.instance());
                ...
                if (output.type === "block") {
                    ...
                    $$outputStack = ItemStackCtorFromBlock(ModAPI.blocks[output.id].getRef(), ${tags.resultQuantity});
                } else {
                    if (!ModAPI.items[output.id]) {
                        console.warn("Item not found: " + output.id);
                        return;
                    }
                    $$outputStack = ItemStackCtorFromItem(ModAPI.items[output.id].getRef(), ${tags.resultQuantity});
                }
                ...
                if (input.type === "block") {
                    ...
                    if (FurnaceRecipesInstance.addSmeltingRecipeForBlock) {
                        FurnaceRecipesInstance.addSmeltingRecipeForBlock(ModAPI.blocks[input.id].getRef(), $$outputStack, ${tags.experience});
                    } else if (FurnaceRecipesInstance.addSmelting) {
                        FurnaceRecipesInstance.addSmelting(ModAPI.blocks[input.id].getRef(), $$outputStack, ${tags.experience});
                    }
                } else {
                    ...
                    if (FurnaceRecipesInstance.addSmelting) {
