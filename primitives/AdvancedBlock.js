...
PRIMITIVES["block_advanced"] = {
    ...
    tags: {
        ...
        model: "put model code here",
        modelTexList: [], // will be filled dynamically
        animatedSpritesheetTexture: false,
        animatedTextureFrameDuration: 1,
        animatedTextureInterpolate: false,
        tickRatio: 10,
        material: [
            'rock', 'air', 'grass', 'ground', 'wood', 'iron', 'anvil', 'water', 'lava', 
            'leaves', 'plants', 'vine', 'sponge', 'cloth', 'fire', 'sand', 'circuits', 
            'carpet', 'glass', 'redstoneLight', 'tnt', 'coral', 'ice', 'packedIce', 
            'snow', 'craftedSnow', 'cactus', 'clay', 'gourd', 'dragonEgg', 'portal', 
    ...
    extractTexturesFromModel: function(jsonString) {
        let model;
        try {
        ...
        if (model.textures) {
            for (const key in model.textures) {
                if (model.textures[key]) textures.add(model.textures[key]);
            }
        }
        if (model.particle) textures.add(model.particle);
        return Array.from(textures);
    },

    modelList: function() {
        if (!this.tags.model) return [];
        const textures = this.extractTexturesFromModel(this.tags.model);
        this.tags.modelTexList = textures;
        return textures;
    },

    asJavaScript: function() {
        // Handler setup
        const constructorHandler = getHandlerCode("BlockConstructor", this.tags.Constructor, []);
        const breakHandler = getHandlerCode("BlockBreak", this.tags.Break, ["$$world", "$$blockpos", "$$blockstate"]);
        const addedHandler = getHandlerCode("BlockAdded", this.tags.Added, ["$$world", "$$blockpos", "$$blockstate"]);
        const neighborHandler = getHandlerCode("BlockNeighbourChange", this.tags.NeighborChange, ["$$world", "$$blockpos", "$$blockstate"], {
            ...
            "1_12": function(args, code) {
                const copy = [...args]; copy[0] = args[1]; copy[1] = args[2]; copy[2] = args[0];
                return `
        ...
        const brokenByPlayerHandler = getHandlerCode("BlockBrokenByPlayer", this.tags.BrokenByPlayer, ["$$world", "$$blockpos", "$$blockstate"]);
        const randomTickHandler = getHandlerCode("BlockRandomTick", this.tags.RandomTick, ["$$world", "$$blockpos", "$$blockstate", "$$random"]);
        const entityCollisionHandler = getHandlerCode("BlockEntityCollision", this.tags.EntityCollided, ["$$world", "$$blockpos", "$$entity"]);
        const getDroppedItemHandler = getHandlerCode("BlockGetDroppedItem", this.tags.GetDroppedItem, ["$$blockstate", "$$random", "$$forture"]);
        const quantityDroppedHandler = getHandlerCode("BlockQuantityDropped", this.tags.QuantityDropped, ["$$random", "$$fortune"]);
...
\`{
    ...
    function $$ServersideBlocks() {
        ...
        function $$internal_reg() {
            ...
            $$blockClass.staticMethods.registerBlock0.method(
                ModAPI.keygen.block("${this.tags.id}"), ModAPI.util.str("${this.tags.id}"), $$cblock
            );
    ...
    ModAPI.addEventListener("lib:asyncsink", async () => {
        ...
        AsyncSink.setFile("resourcepacks/AsyncSinkLib/assets/minecraft/models/block/${this.tags.id}.json", JSON.stringify(${this.tags.model}));
        ...
        // Auto upload textures from modelTexList
        const texList = ${JSON.stringify(this.modelList())};
        for (const texName of texList) {
            const input = document.querySelector(\`input[data-texture-name="\${texName}"]\`);
            if (!input || !input.files[0]) continue;
            const arrayBuffer = await input.files[0].arrayBuffer();
            const b64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)));
            AsyncSink.setFile(\`resourcepacks/AsyncSinkLib/assets/minecraft/textures/blocks/\${texName}.png\`, __b64toAB(b64));
        }
