...
\`{
    ...
    const $$textureFiles = {}; // store uploaded textures

    function $$extractTextures(modelJSON) {
        const texList = [];
        if (!modelJSON.textures) return texList;
        for (let key in modelJSON.textures) {
            const t = modelJSON.textures[key];
            if (!texList.includes(t)) texList.push(t);
        }
        return texList;
    }

    function $$renderTextureListUI(modelJSON) {
        const list = $$extractTextures(modelJSON);
        list.forEach(tex => {
            ...
            input.addEventListener("change", e => {
                ...
                if (file) {
                    ...
                    reader.onload = ev => $$textureFiles[tex] = ev.target.result;
            ...
            resetBtn.addEventListener("click", () => delete $$textureFiles[tex]);
    ...
    // New: render texture list UI
    AsyncSink.addEventListener("model:loaded", (modelJSON) => $$renderTextureListUI(modelJSON));
